...
try {
    ...
    foreach ($produtosCarrinho as $produto) {
        ...
        $stmt = $pdo->prepare("SELECT Qtd_estoque, Cod_Produto FROM Produto_Tipo WHERE Cod_Produto = :Cod_Produto");
        ...
        if ($produtoEstoque['Qtd_estoque'] < $produto['Quantidade']) {
            $produtosInsuficientes[] = $produtoEstoque['Cod_Produto'];
        }
    }

    if (!empty($produtosInsuficientes)) {
        echo 'Estoque insuficiente para os produtos: ' . implode(', ', $produtosInsuficientes);
        $pdo->rollBack();
    } else {
        // Cria o pedido
